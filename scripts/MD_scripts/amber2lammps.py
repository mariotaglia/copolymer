...
class Lammps:
    ...
    def Write_data(self, Basename, Item_list):
        ...
        while Filename in Dir_list:
            Filename = 'data' + str(i) + '.' +str( Basename)
            i = i + 1
        del i
    ...
    def Write_Lammps(self, Basename):
        ...
        L.append(str(self.atoms) + ' atoms\n')
        L.append(str(self.bonds) + ' bonds\n')
        L.append(str(self.angles) + ' angles\n')
        L.append(str(self.dihedrals) + ' dihedrals\n')
        L.append(str(self.impropers) + ' impropers\n\n')

        L.append(str(self.atom_types) + ' atom types\n')
        print('test', self.bonds)
        if self.bonds > 0:
            L.append(str(self.bond_types) + ' bond types\n')
        if self.angles > 0:
            L.append(str(self.angle_types) + ' angle types\n')
        if self.dihedrals > 0:
            L.append(str(self.dihedral_types) + ' dihedral types\n')
        L.append('\n')

        L.append(str(self.xlo) + ' ' + str(self.xhi) + ' xlo xhi\n')
        L.append(str(self.ylo) + ' ' + str(self.yhi) + ' ylo yhi\n')
        L.append(str(self.zlo) + ' ' + str(self.zhi) + ' zlo zhi\n\n')

        if self.atom_types != 0:
            L.append('Masses\n\n')
            for i in range(self.atom_types):
                L.append(str(i+1) + ' ' +str( self.Masses[i]) + '\n')
            L.append('\n')
            ...
            for i in range(self.atom_types):
                ...
                for j in range(len(self.Nonbond_Coeffs[0])):
                    L.append(' ' + str(self.Nonbond_Coeffs[i][j]))
                L.append('\n')
            L.append('\n')
        ...
        if self.bonds != 0 and self.bond_types != 0:
            ...
            for i in range(self.bond_types):
                ...
                for j in range(len(self.Bond_Coeffs[0])):
                    L.append(' ' + str(self.Bond_Coeffs[i][j]))
                L.append('\n')
            L.append('\n')
        ...
        if self.angles != 0 and self.angle_types != 0:
            ...
            for i in range(self.angle_types):
                ...
                for j in range(len(self.Angle_Coeffs[0])):
                    L.append(' ' + str(self.Angle_Coeffs[i][j]))
                L.append('\n')
            L.append('\n')
        ...
        if self.dihedrals != 0 and self.dihedral_types != 0:
            ...
            for i in range(self.dihedral_types):
                ...
                for j in range(len(self.Dihedral_Coeffs[0])):
                    L.append(' ' + str(self.Dihedral_Coeffs[i][j]))
                L.append('\n')
            L.append('\n')
        ...
        if self.dihedrals != 0 and self.dihedral_types != 0:
            ...
            for i in range(self.dihedrals):
                ...
                for j in range(len(self.Dihedrals[0])):
                    L.append(' ' + str(self.Dihedrals[i][j]))
                L.append('\n')
            L.append('\n')
...
class Amber:
    ...
    def Coerce_to_Lammps(self):
        ...
        if self.CRD_is_read and self.TOP_is_read:
            ...
            Shift = 0
         #   if self.__dict__.__contains__(key)('BOX'):
    ...
    def Read_data(self, Filename):
        ...
        if Filename[-4:] == '.crd':
            # This line corresponds to TITLE name in CRD file
            if Lines[0].split() == []:
           # if string.split(Lines[0]) == []:
            ...
            # This line corresponds to TITLE name in TOPOLOGY file
            if string.split(Lines[3]) == []:
                Basename = Filename[:string.find(Filename, '.')]
                Item_list = [Basename]
                print ('Warning: Title not present... Assigning Basename as Title')
            else:
        ...
        for Line in Lines:
            # Vikas' Modification: This condition ignores all the lines starting with % in the topology file.
            if Line[0] != '%':
 #               Item_list.extend(string.split(Line))
    ...
    def Read_CRD(self, Basename):
        ...
        #if self.__dict__.__contains__(key)('ITITL'):
            if Pop(Item_list, 0) != self.ITITL:
                print ('warning: ITITL differs!'),
        else:
            self.ITITL = Pop(Item_list, 0)
        print('the title', self.ITITL ) # Vikas Modification : Priting the Title
        ...
        # Vikas' Modification: Printing number of atoms just to make sure that the program is reading the correct value.
        print ('print number of atoms', self.NATOM)
        ...
        if (len(Item_list) % 3) != 0:
            ...
            self.TIME = 0
        # Vikas' Modification : Printing simulation time, just to make sure that the program is readint the correct value.
        print ('print simulation time',self.TIME)
        ...
        Pop(Item_list, 0)
        Pop(Item_list, 0)
        print('len self Natom', self.NATOM)
        for i in range(self.NATOM):
...
#        else:
        ...
        print( 'tot num of atoms',self.NATOM)
      ...
      #      self.HBCUT.append(eval(Pop(Item_list, 0)))
        ...
        print ('Reading Join Array: Tree joining information')
        self.JOIN = []
        for i in range(self.NATOM):
